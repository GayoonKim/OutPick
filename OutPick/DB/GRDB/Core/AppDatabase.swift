...
final class AppDatabase {
    ...
    func deleteAllUserSessionData() async throws {
        try await dbPool.write { db in
            ...
            try db.execute(sql: "DELETE FROM chatDeletionCleanup")
            try db.execute(sql: "DELETE FROM chatDeletionCursor")
            try db.execute(sql: "DELETE FROM chatDeletedMessageMarker")
