...
- 별도 관리자 작업 목록 projection은 두지 않는다. 관리자 API는 `moderationMessageIncidents`, `moderationUserReports`, `moderationRoomReports`를 직접 필터·정렬하고 actionable message view는 `acceptanceState=reviewable`만 포함한다. `resolveMessageModeration`은 decision별 동일 seq 복원 또는 관리자 삭제, confirmed violation, 조건부 계정 조치, Evidence retention/cleanup과 mutation audit을 일관되게 처리한다.
