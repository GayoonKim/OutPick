...
- 메시지 신고는 전용 `submitMessageReport`가 먼저 `moderationMessageReportPreparations/{preparationID}`와 request별 `moderationMessageReportRequests/{requestID}`, server-only guard/bundle/job을 `processing`으로 만든다. request receipt는 응답 유실 뒤 같은 clientRequestID의 원래 결과를 보존하고, preparation은 새 clientRequestID 재시도가 같은 reporter/message/revision의 결정적 bundle을 재사용하게 한다. 제한된 text snapshot 또는 전체 media evidence가 `available`이 된 뒤에만 canonical `moderationMessageIncidents/{incidentID}`, review revision별 reporter 문서와 작성자 aggregate의 `reportedMessages/{incidentID}`·`messageReporters/{reporterID}` marker를 확정한다. processing/failed는 신고 count·queue·작성자 패턴에 포함하지 않고, accepted 신고의 임계치도 Message visibility를 변경하지 않는다. terminal preparation은 reason/detail과 중복 room/message/source 식별자를 scrub하고 최소 수렴 상태만 createdAt부터 30일 보존한 뒤 TTL 삭제한다. `submitUserReport`는 프로필/참여자 사용자 신고로 유지하며 attachment를 받지 않는다.
- iOS에는 별도 pending report GRDB나 영구 신고 여부 flag를 저장하지 않는다. 같은 화면의 network retry만 동일 UUID/payload를 재사용하고 화면 종료 뒤 사용자가 새로 제출하면 새 UUID를 사용한다. 같은 revision의 중복·진행 중 preparation 재사용과 삭제 선행 여부는 서버 transaction이 권위이며 로컬 메시지 schema에는 신고 상태를 추가하지 않는다.
...
- `chatMessageCleanupJobs/{sha256(roomID:messageID)}`는 reply preview·media index·고정 message Storage prefix를 재시도한다. 계정 탈퇴가 생성·재사용한 job은 server-only `accountDeletionRequestID`로 탈퇴 요청에 연결하며 final verify는 연결 job 전부의 `completed`를 요구한다. completed는 7일 TTL, 20회 실패는 TTL 없이 남긴다.
...
- `roomOwnershipSuccessionJobs/{jobID}` schema v3는 기존 계정 fence와 실행 상태 외에 `phase/cursor/pendingRoomCount/failedRoomCount`를 가진다. `ownedCanonical/ownedLegacy`에서 방별 작업을 만들고 `members`에서 일반 참여방만 페이지 단위로 정리한다. `waitingRooms`에서 모든 방별 작업과 일반 정리가 끝나야 `completed + result=resolved`가 된다. 일부 방 실패 시 `failed + result=partialFailure`로 남고 계정 삭제 finalizer는 진행하지 않는다.
- 부모 탐색/일반 정리 실패는 `failed + result=orchestrationFailure`로 phase·cursor·오류를 보존한다. 이미 생성된 자식은 부모 status가 아니라 자신의 상태·generation·기존 50초 기한·계정 fence로 실행 여부를 판단한다. 미완료 자식이 있으면 실패 부모도 `nextAttemptAt`으로 예약/watchdog 조회에 포함하고, 부모 경로는 새 탐색 없이 만료 정리만 수행한다. 마지막 자식 종료 시 예약을 해제하며 부모 오류는 성공으로 덮지 않는다. 실패 방 수동 replay는 부모 orchestrationFailure를 유지하고 그 방에만 새 generation/50초를 부여한다. 계정 삭제 finalizer는 계속 차단된다.
- 자식 `roomSuccessionAttempts/{roomID}`는 `generation/status/attempt/startedAt/deadlineAt/nextAttemptAt/lease/result/lastErrorCode`를 가진다. 방별 생성 시부터 50초, 최초 포함 최대 4회(실패 후 대기 5/15/20초)다. 권한 transaction은 기한·세대·계정 fence를 재검증하고 방별 완료와 부모 미완료 counter 감소를 원자 저장한다. 기한 후 신규 승계/방 종료를 시작하지 않으며 기존 성공은 보존한다. DB 장애로 실패 기록이 늦어질 수 있다.
- 일반 참여방 정리는 방별 50초에 포함하지 않는다. 실패/진행 중인 owner의 membership은 남겨 후속 승계/운영 처리를 보장한다. 운영 CLI는 실패한 방 하나만 새 generation과 50초 예산으로 재처리한다. 성공한 자식은 `expiresAt` 30일 TTL, 실패한 자식은 TTL 없이 보존하며 모두 client deny다.
...
- `moderationMessageIncidents/{incidentID}`는 review revision별 긴급/전체 고유 신고 principal 수, `queueClass`, review/evidence 상태를 서버 전용으로 집계한다. 일반 단일 신고는 `holding`, 같은 메시지 고유 신고자 2명은 `reviewRequired`, 긴급 단일 신고는 `urgent`다. `holding && reviewDueAt <= serverNow`는 scheduler로 상태를 바꾸지 않고 관리자 직접 조회에 포함한다.
- 같은 작성자의 최근 7일 `reportedMessages` 3개와 `messageReporters` 2개가 함께 조회될 때만 사용자 단위 검토 신호로 사용한다. 두 marker query를 분리해 두 번째 신고자가 네 번째 이전 메시지에만 있어도 놓치지 않으며 각 read는 3개와 2개로 제한한다. 충족 시 `messagePatternReviewUntil = min(세 번째 최신 메시지 시각, 두 번째 최신 신고자 시각) + 7일`을 기록하고 관리자 API가 server now와 비교하므로 만료 scheduler가 필요 없다. 긴급 2명 또는 전체 3명의 24시간 threshold도 queue 우선순위 신호로만 사용하고 message visibility를 변경하지 않는다.
...
- 별도 관리자 작업 목록 projection은 두지 않는다. 관리자 API는 `moderationMessageIncidents`, `moderationUserReports`, `moderationRoomReports`를 직접 필터·정렬하고 actionable message view는 `acceptanceState=reviewable`만 포함한다. Phase 7.5의 `resolveMessageModeration`은 `reviewOutcome`, `contentAction`, `accountAction`을 분리하며 `contentAction=delete`만 일반 삭제 tombstone을 만든다.
- `Rooms/{roomID}.messageDeletionRevision`은 작성자·방 관리자·플랫폼 관리자·계정 탈퇴 정리 등 서버가 실제 visible 메시지를 처음 tombstone으로 바꿀 때만 증가한다. 모든 삭제 진입점은 공통 deletion mutation을 사용한다. 일반·관리자 삭제 `Messages/{messageID}`는 ID/roomID/seq, `isDeleted=true`, `deletionRevision`, `deletedAt`, `senderAnonymized=false`와 sender UID·닉네임·아바타·전송 시각·답장 presentation을 남기고 원문·첨부·룩북·검색 필드를 제거한다. 계정 탈퇴 bulk는 `senderAnonymized=true`, `senderNickname=알 수 없는 사용자`, 전송 시각만 남기고 UID·email·아바타·답장 정보를 제거한다. 두 형태 모두 방이 존재하는 동안 durable deletion delta로 유지하며 iOS는 계정·방별 `lastAppliedDeletionRevision` 이후 tombstone만 조회한다. 별도 deletion journal과 사용자별 deletion inbox는 두지 않는다.
- `chatMessageDeletionDeliveryJobs/{deliveryID}`는 삭제 commit 뒤 Socket fast path만 재시도하는 server-only 임시 outbox다. 단건은 `messageDeleted + roomID + messageID + deletionRevision`, 계정 탈퇴 방 batch는 `deletionHeadAdvanced + roomID + fromRevision + toRevision`의 SHA-256 결정적 ID를 사용한다. tombstone·Room head와 같은 transaction에서 생성하며 Socket watcher가 60초 lease, 최대 10회 지수 backoff로 claim한다. 만료 processing은 같은 attempt로 회수하고 completed/failed는 terminal 시점부터 7일 `expiresAt` TTL로 제거한다. 사용자별 inbox나 영구 deletion journal로 사용하지 않는다.
- 단건 삭제는 메시지별 임시 delivery outbox를 만들고, 계정 탈퇴는 collection group query 결과를 방별 batch로 처리해 revision 범위당 head-advanced outbox 하나만 만든다. outbox는 Socket retry 후 TTL 제거되며 deletion truth나 영구 journal이 아니다.
- iOS GRDB는 `chatDeletionCursor(accountID,roomID,revision)`, `chatDeletedMessageMarker(accountID,roomID,messageID,seq,revision,deletedAt,anonymizesSender)`, `chatDeletionCleanup(kind,path,createdAt)`를 둔다. cursor와 message/reply/FTS/media index/outbox scrub 및 cleanup enqueue는 하나의 transaction이며, marker는 서버 journal 복제가 아니라 표시 보존/계정 익명화 정책을 포함한 재노출 방지용 최소 레코드로 방 로컬 데이터 수명 동안 유지한다.
- Evidence 원본은 클라이언트에 공개되지 않는다. 일반 상세와 별도 열람 API는 incident의 current revision만 허용하고 과거 revision 상세·byte는 제공하지 않는다. reviewRevision은 기각 뒤 새 신고 cycle의 reporter/count/bundle/cleanup race를 격리한다. 열람 API는 active admin·App Check·5분 recent-auth, current revision/bundle/object 보존 상태와 Storage exact generation ownership을 검증하고 read-only signer로 GET 전용 5분 V4 signed URL 하나를 만든다. URL 응답 전 Cloud Logging API의 `EVIDENCE_VIEW_URL_ISSUED` 구조화 write가 성공해야 하며 opaque issuanceID를 signed custom audit query에 포함해 Cloud Storage `DATA_READ` GET/Range와 연결한다.
- Evidence 접근 audit은 Firestore 문서나 TTL collection으로 저장하지 않는다. actorUID와 논리 Evidence 식별자, requestID/issuanceID, 발급·만료 시각·결과와 실제 GET audit만 `asia-northeast3`의 환경별 잠긴 `moderation-evidence-audit` log bucket에 1,095일 보존하고 Log Analytics로 날짜·관리자·incident·revision·object·issuance별 조회한다. URL·signature/query, bucket/path, 원문·bytes·이메일·프로필은 기록하지 않고 일반 `_Default` 접근 범위에 민감 audit 사본을 두지 않는다. Evidence object는 `Cache-Control: private, no-store, max-age=0`를 사용하며 list·prefix·일괄 URL과 Cloud Run 프록시 전달은 두지 않는다.
