...
- 같은 `clientRequestID` 재전송은 기존 processing/accepted/failed 결과를 반환한다. 새 `clientRequestID`로 같은 reporter/message/review revision을 다시 신고하면 processing 중에는 기존 결정적 bundle을 재사용하고 accepted 뒤에는 `alreadyReported`만 반환하며, failed cleanup 뒤에는 새 준비를 허용한다. 이 경로는 count·evidence·rate quota를 중복 증가시키지 않는다.
...
- `moderationMessageGuards/{incidentID}`는 클라이언트가 읽을 수 없는 신고/삭제 경합 원장이다. 삭제 transaction이 먼저 commit되면 `messageAlreadyDeleted`를 반환하고 신고·evidence·count·quota를 만들지 않는다. 신고 preparation transaction이 먼저 commit되면 evidence copy를 예약하고, 뒤이은 삭제는 UI tombstone을 즉시 만들되 ready media cleanup을 copy 완료 또는 terminal failure까지 기다린다. copy 완료 뒤에만 accepted 신고를 확정하고 terminal failure면 신고 집계 없이 hold를 해제한다.
