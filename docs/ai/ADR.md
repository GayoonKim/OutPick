...
## ADR-007: scripts/ai 자동화는 반복이 확인된 뒤 추가한다
...
- 반복이 확인되기 전에는 문서 하네스와 수동 명령으로 유지한다.
- 배포, 삭제, 마이그레이션처럼 되돌리기 어려운 작업은 스크립트가 있더라도 사용자 승인 없이 실행하지 않는다.

이유:

- 너무 이른 자동화는 실제로 쓰이지 않는 스크립트를 늘리고 하네스 유지비를 키운다.
- 반복이 확인된 명령만 스크립트화하면 자동화가 실제 컨텍스트 절감과 실수 방지에 기여한다.

트레이드오프:

- 초기 몇 번은 수동으로 명령을 실행하고 기록해야 한다.
- 대신 안정되지 않은 명령을 성급히 고정하는 위험을 줄인다.

자동화 후보 예:

- `changed-files-summary.sh`: 커밋 전 변경 파일 분류가 반복될 때.
- `check-harness.sh`: 공식/로컬 하네스 분리 점검이 반복될 때.
- `verify-swift.sh`: 같은 Swift 빌드 검증 명령이 반복될 때.
- `verify-functions.sh`: 같은 Functions lint/build 검증이 반복될 때.

재검토 조건:

- 특정 명령이 작업마다 반복되고 인자가 안정되면 `scripts/ai`로 승격한다.
