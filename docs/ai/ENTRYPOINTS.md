...
- 루트 포인터: `HANDOFF.md`
- 완료된 삭제 purge 큐 소진 개선: `docs/ai/tasks/lookbook-deletion-purge-drain/design.md`, `docs/ai/tasks/lookbook-deletion-purge-drain/decisions.md`, `docs/ai/tasks/lookbook-deletion-purge-drain/plan.md`, `docs/ai/tasks/lookbook-deletion-purge-drain/progress.md`, `docs/ai/tasks/lookbook-deletion-purge-drain/qa-checklist.md`, `docs/ai/adr/ADR-018-룩북-영구-삭제는-일일-bounded-drain과-브랜드-lease로-처리한다.md`
- 삭제 purge 큐 소진 서버 경계: `functions/src/index.ts`의 `expiredDeletionRequestPageLoader`, `runLookbookDeletionPurge`, `purgeExpiredLookbookDeletions`, `functions/src/lookbookDeletionPurgeDrain.ts`, `functions/src/lookbookDeletionPurgeLease.ts`, `firestore.indexes.json`
- 삭제 purge 큐 소진 자동 테스트: `functions/src/lookbookDeletionPurgeDrain.test.ts`, `functions/src/lookbookDeletionPurgeLease.test.ts`; `functions/package.json`의 `npm test`가 빌드 후 `lib/*.test.js`를 실행한다.
- 삭제 purge 변경/장애 조사 읽기 순서: task `decisions.md`와 `progress.md` -> `functions/src/lookbookDeletionPurgeDrain.ts` -> `functions/src/index.ts`의 `expiredDeletionRequestPageLoader`와 `purgeExpiredLookbookDeletions` -> `functions/src/lookbookDeletionPurgeLease.ts` -> `firestore.indexes.json` -> 관련 테스트와 `qa-checklist.md`.
- 완료된 삭제 요청 목록 단순화 작업: `docs/ai/tasks/lookbook-deletion-request-list-simplification/design.md`, `docs/ai/tasks/lookbook-deletion-request-list-simplification/plan.md`, `docs/ai/tasks/lookbook-deletion-request-list-simplification/progress.md`, `docs/ai/tasks/lookbook-deletion-request-list-simplification/qa-checklist.md`
- 삭제 요청 목록 단순화 Phase 2~6 구현, Functions 운영 배포, 사용자 수동 QA는 2026-07-13 완료됐다.
- 삭제 요청 목록 단순화 서버/iOS 경계: `functions/src/index.ts`, `functions/src/lookbookDeletionPurgeLease.ts`, `functions/src/lookbookDeletionPurgeLease.test.ts`, `OutPick/Features/Lookbook/Views/Admin/AdminLookbookDeletionManagementView.swift`, `OutPick/Features/Lookbook/ViewModels/AdminLookbookDeletionManagementViewModel.swift`, `OutPick/Features/Lookbook/Domains/Entities/LookbookDeletionRequest.swift`, `OutPick/Features/Lookbook/Repositories/Protocols/LookbookDeletionRepositoryProtocol.swift`, `OutPick/Features/Lookbook/Repositories/Implementations/CloudFunctionsLookbookDeletionRepository.swift`, `OutPick/DB/Firebase/CloudFunctions/CloudFunctionsManager.swift`, `OutPickTests/AdminLookbookDeletionManagementViewModelTests.swift`
