...
- Chat moderator delegation Phase 5 자동 승계·계정 정리: 영구 정지/계정 삭제 transaction → `roomOwnershipSuccessionJobs` → `roomMembershipSweep.ts`의 fence·관리자 전용 후보·no-candidate 종료 → `roomMembershipSweepFunctions.ts`의 Task Queue 5초/15초/30초 예약과 5분 watchdog → account deletion `cleanup.ts`/`drain.ts` finalizer gate·role event 익명화 → iOS `RealtimeSocketService` 동일 ID privacy 갱신. 수동 복구는 `functions/scripts/replay-room-ownership-succession.mjs`, 검증은 `roomMembershipSweep.test.ts`, `chat-moderation.emulator.test.mjs`, `ChatRoomSessionActorTests`/`RealtimeSocketListenerBinderTests`다.
