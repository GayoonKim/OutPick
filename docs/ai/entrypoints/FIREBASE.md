...
- Phase 7.4D 관리자 경계: `functions/src/moderation/admin/{contracts,service,functions,messageResolution,evidenceAccess}.ts`가 message queue/current-revision detail, `resolveMessageModeration`, `issueMessageEvidenceViewURL`을 제공한다. 일반 목록·상세는 active admin+App Check, Evidence URL은 추가 5분 recent-auth, 계정 restriction/suspension 결정은 추가 recent-auth를 요구한다. Evidence API는 logical object ID+exact generation을 받고 bundle/path는 서버가 current revision에서 유도하며, 분리된 read-only signer로 5분 V4 GET을 만든 뒤 Cloud Logging write 성공 후에만 반환한다. `functions/src/moderation/messageEvidence/evidenceCleanup.ts`의 5분 drain은 30일 보존 만료 bundle을 cleanup job으로 전환한다. 발급 로그와 signed opaque issuanceID의 Storage `DATA_READ`는 서울 리전 전용 log bucket에서 1,095일 보존하되 Firestore access-audit collection은 두지 않는다.
...
- Phase 7.4B 로컬 transaction은 `functions/src/moderation/messageEvidence/service.ts`가 소유한다. revision-independent `moderationMessageReportRequests/{requestID}`를 revision 선택 전에 조회하고, text/lookbook은 즉시 accepted, ready media는 preparation/bundle/`moderationEvidenceCopyJobs` processing으로 분리한다. `acceptMessageEvidenceBundleService`가 available bundle의 preparation을 최대 30건씩 draining하며 incident/revision/reporter·작성자 aggregate·각 preparation의 최초 receipt·visibility를 함께 확정한다. 같은 preparation의 추가 UUID receipt는 무제한 bulk write하지 않고 동일 UUID 재조회 시 terminal generation 결과로 개별 수렴한다. 실패 재시작은 partial cleanup이 끝난 preparation/bundle/copy job의 동일 generation을 검증한 뒤 세 문서를 함께 `+1`로 전환한다. `functions/src/index.ts` export와 배포는 아직 없다.
