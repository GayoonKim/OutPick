...
- Development candidate QA는 `gayunkim.1@gmail.com`에 Functions 기본 Compute 계정과 `outpick-lookbook-task-dev` 계정 각각의 리소스 범위 `roles/iam.serviceAccountOpenIdTokenCreator`만 영구 부여한다. IAM Credentials `generateIdToken`으로 1시간 OIDC ID token을 발급하며 전체 access token/JWT/blob 서명이 가능한 사용자 `roles/iam.serviceAccountTokenCreator`, 서비스 계정 key와 Production 영구 binding은 사용하지 않는다.
...
- Codex는 IAM private `lookbookExtractionIssueOpsRead`, `lookbookExtractionIssueOpsWrite`, `verifyLookbookExtractionFix`와 환경 필수 CLI로 allowlist evidence만 조회·변경한다. Production identity는 audience가 있는 환경별 operator service account impersonation을 사용하며, compare-and-set과 audit을 적용한다.
