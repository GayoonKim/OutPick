...
- Phase 4는 Worker IAM 전용 `/runtime-contract`, `/smoke/extraction`과 Functions `verifyLookbookExtractionFix`, 10분 projection reconciler를 사용한다. verifier는 Cloud Run v2 observed traffic 100%, runtime/source revision, 대표 job 실제 재추출과 ground truth를 통과한 경우만 fixed/retry-ready를 기록한다. Production Function service account에는 배포 전 Cloud Run Viewer와 Worker Invoker가 필요하다.
