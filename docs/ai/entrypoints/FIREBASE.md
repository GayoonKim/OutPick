...
- Production canonical 자원명은 Quarantine `outpick-664ae-chat-media-quarantine`, ready `outpick-664ae-chat-media`, image Service `outpick-chat-media-image`, video Job `outpick-chat-media-video`, queue `chat-media-image-processing`/`chat-media-video-processing`, identity `outpick-chat-media-orch`/`outpick-chat-media-cleanup`/`outpick-chat-media-worker`/`outpick-chat-media-task`다. orchestrator ID는 Google Cloud 서비스 계정의 30자 상한을 지키기 위해 `orch`로 축약한다. 생성·IAM·배포 순서와 rollback은 `docs/ai/runbooks/CHAT_MEDIA_PRODUCTION_ROLLOUT.md`를 따른다.
...
- Phase 7.5 deletion cutover 감사는 `functions/scripts/audit-chat-deletion-revisions.mjs --project {projectID}`를 사용한다. Rooms를 열거한 뒤 각 `Messages` 하위 collection에서 `isDeleted=true`를 필요한 projection으로 읽고 revision·정렬 필드·첨부/cleanup·reply/media 잔존을 내용·사용자 식별자 없이 집계한다. Firestore·Storage write는 수행하지 않는다. 최초 Development 결과는 누락 10건/head 0, cleanup failed 10건/media index 잔존 10건, Production 삭제 0건이었다.
- cleanup 실패 원인은 `Messages.replyPreview.messageID` field override가 COLLECTION_GROUP ASC만 보유해 `moderationCleanup.ts`의 COLLECTION query가 `FAILED_PRECONDITION(9)`인 계약 불일치였다. `apply-chat-deletion-field-index.mjs`가 Firestore Admin field-level patch로 Development 두 scope만 exact 보존·추가하고 operation 완료를 기다린다. `repair-chat-deletion-cutover.mjs`는 Development-only·기본 dry-run이며 expected room hash/count/head와 exact confirmation으로 cleanup 재개와 revision backfill을 분리한다.
- 2026-08-27 승인된 Development 적용은 두 scope READY → failed cleanup 10건 attempt 0 재개 → 기존 scheduler 1회 실행·전부 attempt 1 completed/reply·media 잔존 0 → 기존 deletedAt을 보존한 revision 1...10·Room head 10 transaction 순서로 완료했다. 사후 감사는 revision 보유 10/누락 0/max 10=head 10/mismatch 0이다. migration outbox와 Production 변경은 0건이다.

### Phase 2 신고·관리자 처리 경계

- 일반 사용자 callable: `submitMessageReport`, `submitUserReport`, `submitRoomReport` → `functions/src/moderation/reports/functions.ts`. `submitMessageReport`는 Phase 7.5E에서 root export됐고 `parseSubmitMessageReportInput` 뒤 `messageEvidence/service.ts` transaction으로 연결된다. 메시지 신고는 attachment 선택 없이 서버가 전체 manifest를 파생해 canonical message incident, review revision reporter, sender `reportedMessages`/`messageReporters` marker와 공통 evidence bundle을 함께 기록한다.
...
- Phase 7.5A~B 관리자·삭제 경계: `functions/src/moderation/admin/{contracts,service,functions,messageResolution,evidenceAccess}.ts`가 message queue/current-revision detail, 분리된 `reviewOutcome + contentAction + accountAction`의 `resolveMessageModeration`, `issueMessageEvidenceViewURL`을 제공한다. `contentAction=delete`는 자체 삭제 patch가 아니라 `functions/src/chat/deletion/mutation.ts`를 같은 incident/evidence/제재 transaction 안에서 호출한다. 작성자·방 관리자 `functions/src/chat/moderation/service.ts`와 계정 탈퇴 `functions/src/accountDeletion/{cleanup,drain}.ts`도 같은 mutation 규칙을 사용한다. Room `lastMessage` 삭제 요약은 공용 `DELETED_MESSAGE_PREVIEW = "삭제된 메시지입니다"`를 사용하며 계정 탈퇴의 고아 preview 정리도 같은 상수를 재사용한다. 일반 목록·상세는 active admin+App Check, Evidence URL은 추가 5분 recent-auth, 계정 restriction/suspension 결정은 추가 recent-auth를 요구한다.
- Phase 7.5B~C 데이터·전달 경계: 단건은 최소 message tombstone·Room `messageDeletionRevision +1`·message outbox를, 계정 탈퇴는 최대 30건 방 batch의 연속 revision·head-advanced outbox를 원자 생성한다. `chatMessageCleanupJobs.accountDeletionRequestID`는 탈퇴 완료를 reply/media/Storage cleanup 완료와 연결한다. `chatMessageDeletionDeliveryJobs`는 client deny-all·7일 `expiresAt` TTL을 유지하고 `Socket/src/deletion/deletionDeliveryWatcher.js`가 `status + nextAttemptAt|leaseExpiresAt` 인덱스로 due/만료 job을 복구해 대상 room에만 emit한다. 별도 Functions→Socket HTTP API는 없다.
- 2026-08-26 Development `outpick-test`에 Rules, exact index 5개, preparation TTL, Function 7개와 signer 최소 IAM을 배포했다. 전용 `moderation-evidence-audit` bucket은 `asia-northeast3`, Analytics, retention 1,095일, `locked=true`다. Storage DATA_READ는 GCP 제약상 프로젝트의 `storage.googleapis.com` 전체에 활성화하고, Evidence GET만 전용 sink에 포함하며 `_Default`에서는 Evidence app log와 모든 GCS DATA_READ를 두 exact exclusion으로 제외한다. 일회성 fixture의 실제 URL 발급·전체/Range GET에서 같은 issuanceID의 발급 event 1건·Storage GET 2건이 전용 view에 연결되고 `_Default`는 0건이었다. 임시 Auth/Firestore/Storage/App Check·rate bucket은 잔여 0건으로 정리하고 기존 Auth 2명·active platform admin 0명으로 원복한 뒤 bucket을 마지막에 잠갔다. Production은 미변경이다.
- 같은 날 잔여 QA에서 유효 PNG와 정확히 350 MiB인 재생 가능 MP4의 원격 probe·전체 GET·앞/뒤 Range, 실제 5분 만료 후 HTTP 400 거부, recent-auth 갱신 뒤 새 URL·새 issuanceID 재발급을 통과했다. 운영 필드 결합 조회·내림차순과 issuance별 GET 6/1/1건, `_Default=0`을 확인했다. 영구 App Check debug token은 만들지 않았고 임시 Auth/Firestore/Storage/rate bucket 잔여 0, 기존 Auth 2명·active platform admin 0명, 최근 Function ERROR 0으로 종료했다.
- 기존 사용자·방 신고 transaction은 aggregate/submission/reporter/rate bucket을 원자적으로 갱신한다. Phase 7.4 메시지 신고는 preparation transaction이 `moderationMessageGuards/{incidentID}`를 삭제 transaction과 함께 읽고 써 first-commit-wins를 보장하고, evidence available 뒤 accepted 확정 transaction만 incident/reporter/aggregate를 갱신한다. 정확히 같은 clientRequestID transport replay는 무료이고, 이전에 보지 못한 새 UUID receipt는 preparation 재사용·semantic duplicate·삭제 우선 결과여도 user/room 요청과 공유하는 1분 10회 transport limiter를 소비한다. moderation count와 evidence는 중복 증가하지 않는다.
...
- Phase 7.5A 순수 계약은 `functions/src/moderation/messageEvidence/contracts.ts`와 `contracts.test.ts`에서 확인한다. versioned canonical tuple ID, 최초 revision 0, `processing → evidence available → accepted`, queue의 24시간 고유 reporter count/7일 작성자 패턴, account action별 retention을 Firebase SDK 없이 계산한다. 자동 hide 결과는 반환하지 않는다.
- Phase 7.5A transaction은 `functions/src/moderation/messageEvidence/service.ts`가 소유한다. revision-independent `moderationMessageReportRequests/{requestID}`를 revision 선택 전에 조회하고, text/lookbook은 즉시 accepted, ready media는 preparation/bundle/`moderationEvidenceCopyJobs` processing으로 분리한다. `acceptMessageEvidenceBundleService`가 available bundle의 preparation을 최대 30건씩 draining하며 incident/revision/reporter·작성자 aggregate·각 preparation의 최초 receipt·queue count를 함께 확정하되 Message visibility를 쓰지 않는다. 같은 preparation의 추가 UUID receipt는 동일 UUID 재조회 시 terminal generation 결과로 개별 수렴한다.
- Phase 7.4C worker는 `functions/src/moderation/messageEvidence/{evidenceCopy,evidenceStorage,evidenceCleanup,evidenceFunctions,evidenceRuntime}.ts`가 소유하고 `functions/src/index.ts`가 `onMessageEvidenceCopyQueued`, `onMessageEvidenceCleanupQueued`, `drainMessageEvidenceJobs`를 export한다. ready bucket·message display path·generation·bytes 상한·image/video 개수/MIME를 exact 검증하고, `{bundleID}/g{attemptGeneration}/{attachmentID}/display` create-if-absent와 generation-precondition delete를 사용한다. copy는 최초 포함 3회(즉시, 1분, 2분)이며 9분 timeout/12분 lease, generation+lease fence, 실행당 preparation 최대 30건 drain을 적용한다. 3회 실패는 partial evidence 전부 삭제 후 신고 실패와 public cleanup 해제로 수렴하고 retention cleanup은 attachment와 exact 일치하는 객체를 모두 삭제한 뒤 bundle을 완전 삭제한다.
- Development runtime은 `outpick-test-moderation-evidence`와 전용 `outpick-msg-evidence-dev@outpick-test.iam.gserviceaccount.com`을 사용한다. 버킷은 `asia-northeast3` Standard, UBLA/public access prevention, soft delete 0초, versioning off이며 버킷 IAM은 evidence object manager와 object read 없는 bucket operator만 가진다. Eventarc 두 서비스와 scheduler에는 전용 계정의 서비스별 Run invoker만 부여한다. `firestore.indexes.json`의 preparation `bundleID+status+requestedAt`과 copy/cleanup `status+nextAttemptAt` 인덱스 3개가 Development READY다. Rules·TTL field override·관리자 조회와 Production 리소스는 후속 승인 gate다.
- 2026-08-28 표시 메타데이터 보존/계정 탈퇴 익명화 deletion 계약을 위해 Development `outpick-test`에 `deleteChatMessage`, `resolveMessageModeration`, `finalizeExpiredAccountDeletions` 세 함수만 exact-target 배포했다. 모두 Node.js 24 `ACTIVE`; callable 두 함수는 256MiB/60초, finalizer는 1GiB/540초이고 Scheduler는 매시간·Asia/Seoul·`ENABLED`다. 배포 직후 세 service ERROR는 0건이며 Production은 변경하지 않았다.
- 배포 뒤 Development room `P75 QA 0827`의 작성자 답장 메시지 seq 9를 실제 삭제했다. 공개 tombstone은 `isDeleted=true`, `deletionRevision=2`와 sender UID·닉네임·아바타·전송 시각·replyPreview를 보존하고 msg·attachments를 제거했다. Room `messageDeletionRevision=2`, 결정적 delivery outbox `completed/attempt=1`이며 활성 방 앱에는 재진입 없이 즉시 반영됐다. Production은 변경하지 않았다.
...
- Production TTL은 개인정보 보존 목적·기간 승인 전 활성화하지 않는다. Phase 7 메시지 신고는 attachment 선택이나 동영상 timestamp 없이 텍스트와 이미지 묶음 전체 또는 동영상 전체를 공통 evidence bundle로 복사하고 처리 결과별 retention cleanup을 사용한다. 일반 단일은 `holding`, 같은 메시지 고유 2명은 `reviewRequired`, `reviewDueAt`이 지난 holding은 scheduler 없이 관리자 조회에 포함한다. 작성자 패턴은 서로 다른 메시지 3개와 전체 고유 신고자 2명/7일을 모두 요구한다. 긴급 단일은 즉시 `urgent`이며 긴급 2명 또는 전체 3명/24시간도 queue 신호일 뿐 message visibility를 변경하지 않는다. 신고 처리에서는 명시적 관리자 content delete만 일반 tombstone을 만든다. 모든 서버 확정 삭제는 비식별 tombstone과 Deletion Sync를 사용하고, 단건은 message outbox, 계정 탈퇴 collection group bulk는 방별 revision 범위의 head-advanced outbox를 생성한다.
- 2026-08-27 Phase 7.6 Development에 `chatMessageDeletionDeliveryJobs` client deny-all Rules, `status+nextAttemptAt`·`status+leaseExpiresAt` 복합 인덱스 2개와 `expiresAt` TTL을 exact 반영했다. 인덱스는 `READY`, TTL은 `ACTIVE`다. `submitMessageReport`, `deleteChatMessage`, `resolveMessageModeration`, `finalizeExpiredAccountDeletions` 네 함수는 Node.js 24 `ACTIVE`다. 2026-08-28 실제 텍스트 신고에서 transaction 내부 `revisions/{revision}/reporters` urgent 24시간 조회에 필요한 `reporters(priorityClass ASC, createdAt DESC, __name__ DESC)` COLLECTION 인덱스 누락을 발견해 manifest와 Development exact index `CICAgNjp84oK`에 추가하고 `READY`를 확인했다. 전체 index manifest의 기존 drift 때문에 일괄 `firestore:indexes` deploy는 하지 않았고 Production은 미변경이다.
- 2026-08-28 활성 방 이미지 실시간 수신 QA에서 Development 원격에 manifest의 `chatMediaDeliveryJobs(status, nextAttemptAt)`와 `(status, leaseExpiresAt)` COLLECTION 복합 인덱스가 누락된 drift를 확인했다. `Socket/src/media/mediaDeliveryWatcher.js`는 pending/retryPending/만료 processing 쿼리를 `Promise.all`로 drain하므로 한 인덱스의 `FAILED_PRECONDITION`이 pending delivery까지 중단시킨다. Development exact index `CICAgNi4t5oK`, `CICAgOj3gpIK`만 생성해 둘 다 `READY`로 만들었고, 기존 pending job은 `attempt=1/completed`로 자동 수렴했으며 이후 watcher 오류는 없다. manifest는 이미 정확해 추가 파일 변경은 없고 Production은 미변경이다.
- 같은 QA의 seq 6 이미지 신고는 Development `onChatMediaWorkerCompleted` 배포본이 현재 `functions/src/chat/media/readyService.ts`보다 오래돼 Message/mediaIndex attachment에 evidence copy가 요구하는 `generationOriginal/contentTypeOriginal`을 쓰지 않은 deployment drift로 `failed-precondition` 400이 됐다. 요청 receipt와 transaction write는 생성되지 않았다. 현재 코드의 writer 회귀와 Functions 245/245·build·lint 오류 0을 확인한 뒤 trigger 하나만 Development에 재배포해 Node.js 24 `ACTIVE`와 기존 전용 runtime identity를 확인했다. 기존 seq 6은 terminal `MediaUploads.normalizedManifest`와 message/mediaIndex의 ID·seq·attachment ID·bucket·path·bytes를 transaction에서 exact 대조한 뒤 누락 필드만 보정했다. Production은 미변경이다.
