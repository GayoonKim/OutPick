...
- 상태: Phase 2.5 `ChatMessage` 모델 파일 분리 완료.
- 공식 하네스 반영 완료:
  - `docs/ai/PRD.md`
  - `docs/ai/FLOW.md`
...
- `Socket/package.json`에 `start`, `check` script를 추가했다.
...
1. Phase 3 소켓 서버 `chat:lookbookShare` 이벤트 구현 전 서버 파일과 보안 경계를 재확인한다.
2. 서버 payload shape, size validation, room membership, closed room 검증을 구현한다.
3. 정상 ACK와 실패 ACK가 iOS `LookbookChatShareAckMapper`와 맞는지 로컬 서버로 확인한다.
4. 운영 배포는 별도 승인 전까지 하지 않는다.
...
- `SocketIOManager.sendLookbookShare(...)`는 `chat:lookbookShare` payload에 `messageType`, generic `msg`, `sharedContent`, sender snapshot, `messageID`, `sentAt`을 담는다.
