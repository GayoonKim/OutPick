...
- [x] Phase 7 Production 사전 읽기 전용 감사에서 message·삭제 tombstone·deletion delivery·Evidence 데이터 0건을 확인해 migration 없이 진행했다. Rules source hash, composite 61/61 READY, TTL 22/22 ACTIVE, exact Function 11/11 ACTIVE와 scheduler 2/2 ENABLED를 확인했다.
- [x] Production Evidence 최소 IAM·UBLA/PAP·Storage DATA_READ·전용 sink/view·`_Default` exclusions를 확인했다. 격리 callable/전체 GET/Range smoke와 issuance audit 1건·GET 2건·`_Default` 0, 임시 잔여 0을 통과한 뒤 서울 Analytics bucket을 1,095일·`locked=true`로 잠갔다.
- [x] Production Socket `outpick-socket-p75-del-0828` 0% candidate의 readiness·인증 handshake·runtime 계약·ERROR 0을 확인한 뒤 100% 전환했다. canonical readiness·인증 handshake·ERROR 0과 Phase 7 데이터/job 잔여 0을 사후 감사했고 `outpick-socket-p73-prod-0820`을 0% rollback으로 보존했다.
- [x] Development 상대 프로필 이미지 미표시 원인을 기본 Storage Rules의 구버전 cross-user 3문서 조회로 확인했다. Firestore Rules·프로필/active projection·avatar 객체는 정상이었고, 최신 profile Storage Rules 5/5 뒤 기본 Storage ruleset `c44c38b3-713f-49b8-9db2-814d73a15dcd`만 배포해 로컬·Production source hash 일치와 채팅 메시지·설정 참여자 이미지 정상 표시를 확인했다.
