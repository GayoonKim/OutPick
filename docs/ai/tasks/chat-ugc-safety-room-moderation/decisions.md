...
- 같은 `clientRequestID` 재전송은 transport replay로 보고 `processing`이면 같은 준비 작업을, `accepted`면 기존 접수 결과를 반환한다. 같은 reporter/message/reviewRevision의 새 `clientRequestID`는 준비 중이면 기존 결정적 bundle 작업을 이어서 확인하고, 이미 접수됐으면 `alreadyReported`를 반환해 앱이 `이미 신고한 메시지예요`를 표시한다. 이 경로들은 evidence·신고 집계·moderation quota를 중복 증가시키지 않는다.
...
- 삭제가 먼저 commit되면 신고 문서·aggregate·rate quota·evidence를 만들지 않고 `messageAlreadyDeleted`와 messageID/seq를 반환한다. 앱은 `이미 삭제된 메시지예요`를 표시하고 같은 messageID/seq를 즉시 삭제 tombstone으로 갱신한다.
