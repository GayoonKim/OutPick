...
## Phase 7.4D 실행 경계 확정 — 2026-08-26

- Phase 7.4D는 Rules·index/TTL reconcile·관리자 query·감사된 Evidence 단건 조회의 로컬 구현과 자동 테스트를 먼저 완료하고 중단한다. 변경 대상과 rollback을 다시 감사한 뒤 사용자 별도 승인을 받은 경우에만 Development에 exact 배포한다.
- 기존 Development index/field override drift가 있으므로 전체 `firestore:indexes` manifest를 일괄 동기화하지 않는다. Production Rules·index/TTL·Functions·Storage/IAM과 외부 활성화는 별도 승인 gate로 유지한다.
- Evidence byte는 Cloud Run 프록시 스트리밍 없이 5분 만료의 단건 generation 고정 V4 signed GET URL로 전달한다. 일반 관리자 상세 응답은 상태와 논리 object ID만 반환하고 URL을 포함하지 않는다. 별도 열람 API는 활성 플랫폼 관리자·App Check·`auth_time` 5분 이내를 요구하고 기존 관리자 read limiter 120회/분을 적용하며, URL마다 서버 생성 `issuanceID`를 새로 만든다. list·prefix·일괄 URL 발급과 write/delete 권한은 허용하지 않는다.
- 열람 API와 일반 message 상세는 incident의 현재 `reviewRevision`만 허용한다. revision은 과거 Evidence 열람용이 아니라 기각 뒤 새 유효 신고 회차의 reporter/count/bundle/cleanup race를 분리하는 내부 cycle ID다. `dismissed` 과거 revision은 즉시 cleanup 대상이고 위반 확정 revision은 메시지가 삭제돼 새 revision이 열리지 않으므로 과거 revision byte 열람 목적이 없다. 요청 revision 불일치, `cleanupPending|deleting|deleted`, due-for-delete, stale generation과 다른 incident/bundle/object는 거부한다.
- Evidence URL 서명은 copy/cleanup runtime과 분리한 환경별 read-only service account가 담당한다. 전용 계정은 Evidence object get과 자기 자신에 대한 `iam.serviceAccounts.signBlob`, URL 발급에 필요한 최소 Firestore read·Logging write만 가지며 JSON key, object list/create/update/delete 권한은 갖지 않는다.
- Evidence 접근 감사는 Firestore `moderationAuditLogs` 또는 별도 access-audit 문서로 저장하지 않는다. 서버는 signed URL을 메모리에서 만든 뒤 Cloud Logging API로 구조화 `EVIDENCE_VIEW_URL_ISSUED`를 직접 쓰고 성공을 기다리며, log write 실패 시 URL을 응답하지 않는다. 로그는 `issuanceID`, actorUID, incidentID, reviewRevision, bundleID, 논리 object ID, objectGeneration, requestID, issuedAt, urlExpiresAt, result만 저장한다. 재발급은 새 issuanceID와 새 발급 로그를 만든다.
- signed URL에는 actorUID 대신 opaque `x-goog-custom-audit-issuance-id={issuanceID}` query parameter를 서명해 넣는다. Evidence bucket의 Cloud Storage `DATA_READ` audit logging을 명시적으로 활성화하고 실제 GET/Range 로그의 custom audit field를 발급 로그와 issuanceID로 연결한다. Range GET 여러 건은 같은 issuanceID의 한 열람 세션으로 분석하되, URL을 공유받은 제3자의 실제 신원까지 증명하지는 못한다.
