...
- 코드 파일 추가, 수정, 이동, 삭제처럼 구현 변화가 생기면 반드시 `docs/ai/ENTRYPOINTS.md`와 관련 `docs/ai/entrypoints/{domain}.md` 또는 데이터/API/아키텍처 하네스를 최신화한다.
- 하네스 최신화는 다음 작업자가 "어느 파일을 보면 원하는 정보를 알 수 있는지" 답할 수 있게 코드 진입점, API/데이터 계약, DI/Coordinator 연결, 검증/QA 위치를 남기는 것을 완료 기준으로 한다.
